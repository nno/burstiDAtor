...
public class Density extends Histogram {
    public Density(Spikes spikes) {
        ...
        SpikeDelta full_duration = new SpikeDelta(interval_start,
                interval_end);
        System.out.println("" + full_duration + "/" + interval_start + "/"
                + interval_end);
        int n_intervals = getInterval(interval_start, full_duration, n_spikes,
                end_spike) + 1;
        int[] interval_counts = new int[n_intervals];

        for (int i = 0; i < spikes.length(); i++) {
            Spike spike = spikes.get(i);
            int interval = getInterval(interval_start, full_duration, n_spikes,
                    spike);
    ...
    public static void main(String... unused) throws Exception {
         ...
         * 
         * 
         * String[] fns = {"500 180.txt", "250 180.txt" };
         */
