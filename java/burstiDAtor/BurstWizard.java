...
public class BurstWizard {
     ...
     */
    public static void processDirectory(File dir, BurstiDAtor parent) throws Exception {
	...
	String prefix = String.format("%s%s%s%s_%s", path, File.separator, pf, t, type);
	...
	for (int i = 0; i < n; i++) {
		...
		File acfileout = Utils.getCanonicalOutputFile(file, "autocorrappend");
		BufferedImage imga = ac.getPlot();
		ac.writeOutputFile(acfileout, imga);

		Density density = new Density(sp);
		File densityfileout = Utils.getCanonicalOutputFile(file, "densityappend");
	...
	final String[] shortsummaryfields = Settings.getInstance().getS("shortsummaryfields").split(",");
    ...
    public static void burstDirectoryWizard(String rootdir, BurstiDAtor parent) throws Exception {
	burstDirectoryWizard(rootdir, true, parent);
    }

    public static void burstDirectoryWizard(BurstiDAtor parent) throws Exception {
