...
public class Histogram {
	...
	for (int i=0; i<n_bins; i++) {
		...
		g2d.drawString(label, (int) (x - (w/2) + (dx/2)), y - h/2);
	    }
	    ...
	    g2d.drawLine(x, my0+my1, x, my0+my1 + (is_major ? tickmajor : tickminor));

	    if (is_major) {
		// draw label
		String label = i==0 ? "0" : String.format("%.1f", bins.get(i).doubleValue());
		int w = fm.stringWidth(label);
		int h = font_size;
	...
	for (int c=0; c<=max_y; c++) {
		...
		g2d.drawString(label, x - tickmajor - w - tickminor, y + h/2);
	    }
	}
    ...
    public void writeOutputFile(File file, BufferedImage img) throws Exception {
	try {
